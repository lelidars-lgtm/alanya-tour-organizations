(() => {
  'use strict';
  const rawCfg = window.ATO_CONFIG || window.ATO_BOOKING_CONFIG || {};
  const cfg = {
    supabaseUrl: rawCfg.supabaseUrl || rawCfg.supabaseURL || '',
    supabasePublishableKey:
      rawCfg.supabasePublishableKey || rawCfg.supabaseAnonKey || rawCfg.supabaseKey || '',
    siteBaseUrl: rawCfg.siteBaseUrl || location.origin
  };
  let client = null;
  ...
  window.ATOBooking={
    ...(window.ATOBooking || {}),
    configured,getClient,createTripRequest,getPublicBooking,
    signIn,signOut,session,listBookings,updateBooking,ticketUrl,cleanPhone
  };
})();
