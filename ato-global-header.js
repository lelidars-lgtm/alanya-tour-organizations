...
/* V3.1 — hard guard against legacy language code appending a second visible label.
   Keeps exactly one text label per premium grid row without changing icons, links,
   dropdown layout, promo ribbon or mobile behavior. */
function atoRemoveLegacyGridDuplicates(){
 const root=document.getElementById('atoGlobalHeaderRoot');if(!root)return;
 root.querySelectorAll('.ato-grid-row').forEach(row=>{
   const label=row.querySelector('.ato-grid-label,.ato-dropdown-label');
   if(!label)return;
...
atoRemoveLegacyGridDuplicates();
const atoHeaderDedupeObserver=new MutationObserver(()=>atoRemoveLegacyGridDuplicates());
atoHeaderDedupeObserver.observe(document.getElementById('atoGlobalHeaderRoot'),{childList:true,subtree:true});
...
function atoSetDirectText(el,text){if(!el)return;const label=el.querySelector?.('.ato-grid-label,.ato-dropdown-label');if(label){label.textContent=text;[...el.childNodes].filter(n=>n.nodeType===3&&n.textContent.trim()).forEach(n=>n.remove());return;}[...el.childNodes].filter(n=>n.nodeType===3).forEach(n=>n.remove());el.append(document.createTextNode(text))}
