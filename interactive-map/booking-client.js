(function () {
  ...
  const makeNumber = () =>
    `ATO-${new Date().toISOString().slice(0, 10).replaceAll('-', '')}-${crypto
      .randomUUID()
      .slice(0, 8)
      .toUpperCase()}`;
  ...
  async function createPending(payload) {
    if (!configured()) {
      throw new Error(
        'Booking Manager is not connected yet. Add the Supabase URL and publishable key to booking-config.js.'
      );
    }

    const requestNumber = payload.request_number || makeNumber();
    const response = await fetch(
      `${String(cfg.supabaseUrl).replace(/\/$/, '')}/rest/v1/bookings`,
      {
        method: 'POST',
        headers: {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Booking save failed (${response.status})${details ? `: ${details}` : '.'}`
      );
    }

    return { request_number: requestNumber };
  }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    field.innerHTML =
      '<label for="requestPhone">WHATSAPP NUMBER *</label>' +
