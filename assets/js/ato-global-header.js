/* ATO GLOBAL HEADER v1.0 — generated from latest INDEX on 2026-08-17.
...
function atoSetDirectText(el,text){if(!el)return;[...el.childNodes].filter(n=>n.nodeType===3).forEach(n=>n.remove());el.append(document.createTextNode(text))}
function atoApplyHeaderLanguage(lang=atoHeaderLanguage()){
 ...
 q('.nav > a[href="/special-offers.html"] span:last-child').textContent=t.offers;
 ...
 const label=q('.language-dropdown > span'); if(label)label.textContent=window.innerWidth>980?(lang.charAt(0).toUpperCase()+lang.slice(1).toLowerCase()):lang.toUpperCase();
 document.querySelectorAll('#atoGlobalHeaderRoot .language-menu a[data-lang]').forEach(a=>{const code=a.querySelector('.ato-lang-code,.ato-v8-lang-code');if(code)code.textContent=window.innerWidth>980?(a.dataset.lang.charAt(0).toUpperCase()+a.dataset.lang.slice(1).toLowerCase()):a.dataset.lang.toUpperCase()});
